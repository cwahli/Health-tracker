      ...
      if (hasImage || isEditOrText) {
        ...
        if (ctx.isModifySession && (ctx.activeMeal || (ctx.req.body.activeScoutItems && ctx.req.body.activeScoutItems.length > 0))) {
          ...
          const priorSummary = priorMealItems.map((it: any, idx: number) => {
            ...
            return `Dish ${idx + 1}${photoLabel}: ${it.originalName || it.keyword || it.name || 'Dish'} (${it.estimatedWeightGrams || it.weightGrams || 100}g)${it.rawNutritionLabel ? ` [OCR Nutrition Label: ${it.calories ?? it.nutrients?.calories ?? ''} kcal]` : ''}${(it.userSelectedWeight || it.portionChoiceApplied || (Array.isArray(ctx.activeMeal?.userLockedSlots) && ctx.activeMeal.userLockedSlots.some((l: any) => (l.scoutIndex === idx || l.scoutIndex === it.scoutIndex) && l.field === 'weight'))) ? ` [User Selected Portion: ${it.weightGrams}g]` : ''}${compStr ? ` [Ingredients: ${compStr}]` : ''}`;
          }).join('; ');
