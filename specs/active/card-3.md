...
# card-3 — Remove stray v304-scratch.md from repository root

## Goal
Delete the single stray file `v304-scratch.md` at the repository root; change nothing else.
...
- Evidence: `git ls-files v304-scratch.md` lists exactly one root-level file.
...
- Broken input → correct output: after the commit, `git ls-files v304-scratch.md` is empty.
- Adjacent input → unchanged: `git diff --name-only HEAD~1` contains only `v304-scratch.md`.

## In scope
- Delete `v304-scratch.md`.

## Out of scope
- Everything else in the repository.

## Done when
1. `git ls-files v304-scratch.md` exits empty on the branch.
2. `git diff --name-only` of the commit is exactly `v304-scratch.md`.
