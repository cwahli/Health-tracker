...
allowed_files:
  ...
  - tests/lane-contract-freebuff.test.ts
...
gate:
  - npx tsc --noEmit
  - npx vitest run tests/lane-contract.test.ts tests/lane-contract-freebuff.test.ts
  - node scripts/assert-lane-contract.mjs
  - (cd tools/telegram-provider-router && node --check src/index.js && node --test scripts/test-freebuff-model-sync.mjs scripts/test-freebuff-tg-lane.mjs scripts/test-allowance-table.mjs)
