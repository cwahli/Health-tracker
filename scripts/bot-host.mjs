...
export function formatFreemodelWithDepletion(entries, annotated, { current, location, canonical = null, tableLanes = [] } = {}) {
  ...
  // high first, light second — with each row's bakeoff score. Tiers and scores
  // both come from the shared free-catalogs module (catalog Ranked picks,
  // bakeoff ledger verdicts); unlisted renders inside Light because
  // fallback-class is what unlisted means here, and one list beats three. The
  ...
  const tierOfRow = (r) => (tierForModel(refOfRow(r)).tier === 'high' ? 'high' : 'light');
  const coding = usable.filter((r) => tierOfRow(r) === 'high');
  const light = usable.filter((r) => tierOfRow(r) !== 'high');
  if (coding.length) {
    lines.push('', 'Coding-capable:');
    for (const r of coding) lines.push(`• ${r.laneLabel || r.label} — ${scoreLabelFor(refOfRow(r))}`);
  }
  if (light.length) {
    lines.push('', 'Light / fallback:');
    for (const r of light) lines.push(`• ${r.laneLabel || r.label} — ${scoreLabelFor(refOfRow(r))}`);
  }
  ...
  for (const r of rows) {
    ...
    // The benchmark score rides on the button, so the choice is made with the
    // number in front of you rather than from memory: the bakeoff ledger's own
    // verdict, or "unranked" where it has no row.
...
async function handleMessage({ api, config, throttle, sessions, prefs, caches, running, lastUsage, totals, health, bootedAt, busy, message }) {
  ...
  try {
    ...
    if (laneChoice.displaced) {
      // The ledger's reason usually already carries its own reset stamp, so
      ...
      // switch line. `why` is the ledger's own reason, never a raw provider
      // envelope, so this line cannot become the failure QS-2 forbids.
