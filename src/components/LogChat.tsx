...
import { FoodSearchMatchesDropdown } from './FoodSearchMatchesDropdown';
import { StagedMealComposeTray } from './StagedMealComposeTray';
import { DataUsedByAgentPanel } from './DataUsedByAgentPanel';
import { ConsolidatedLogModals } from './ConsolidatedLogModals';
...
${logsText}`);
  ...
  return (
      <div className={`fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex flex-col justify-end sm:justify-center animation-fade-in font-sans ${isFullscreen ? 'p-0' : 'p-0 sm:p-4'}`}>
        ...
        <div ref={chatWindowRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50 dark:bg-slate-900/20">
          ...
          <DataUsedByAgentPanel
            ...
            profile={profile}
            reviewBiomarkerKey={reviewBiomarkerKey}
            dataReviewBatchKeys={dataReviewBatchKeys}
            biomarkers={biomarkers}
            biomarkerHistory={biomarkerHistory}
            dataReviewBatchIdx={dataReviewBatchIdx}
            localBatchSize={localBatchSize}
            setLocalBatchSize={setLocalBatchSize}
            budget={budget}
            setBudget={setBudget}
            currency={currency}
            ...
            setLastSentPayload={setLastSentPayload}
        ...
        <div className="bg-theme-bg-card border-t border-theme-border/80 p-3 flex flex-col gap-2 shrink-0 relative">
          <FoodSearchMatchesDropdown
            catalogMatches={catalogMatches}
            matchingPreviousLogs={matchingPreviousLogs}
            explicitFoodTags={explicitFoodTags}
            activeFoodLogs={activeFoodLogs}
            activeSearchTerms={activeSearchTerms}
            inputText={inputText}
            tagPortionPreFill={tagPortionPreFill}
            t={t}
            setExplicitFoodTags={setExplicitFoodTags}
            setStagedQuery={setStagedQuery}
            setInputText={setInputText}
            setCatalogMatches={setCatalogMatches}
            setActiveSearchTerms={setActiveSearchTerms}
          />
          <StagedMealComposeTray
            explicitFoodTags={explicitFoodTags}
            activeFoodLogs={activeFoodLogs}
            t={t}
            setExplicitFoodTags={setExplicitFoodTags}
      ...
      <ConsolidatedLogModals
        activeModalTableRows={activeModalTableRows}
        activeModalTitle={activeModalTitle}
        setActiveModalTableRows={setActiveModalTableRows}
        fullScreenJson={fullScreenJson}
        setFullScreenJson={setFullScreenJson}
        t={t}
      />
