...
export const nutrientKeySlug = canonicalNutrientKey;

export function isLimitNutrient(key: string): boolean {
  if (!key) return false;
  const clean = canonicalNutrientKey(key);
  return ["calories", "saturatedfat", "transfat", "sodium", "addedsugar", "sugar"].includes(clean);
}

export function isNutrientOverLimit(key: string, value: number, targets: any): boolean {
  if (!key || !targets) return false;
  const target = targets[key] || targets[canonicalNutrientKey(key)];
  if (!target) return false;
  return isLimitNutrient(key) ? value > target : false;
}

export function isNutrientGoalMet(key: string, value: number, targets: any): boolean {
  if (!key || !targets) return false;
  const target = targets[key] || targets[canonicalNutrientKey(key)];
  if (!target) return false;
  return !isLimitNutrient(key) ? value >= target : value <= target;
}

export function lookupByNutrientKey(targets: any, key: string): any {
  if (!targets) return null;
  return targets[key] ?? targets[canonicalNutrientKey(key)] ?? null;
}
