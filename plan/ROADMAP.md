...
| ID | Item | Status / next |
|---|---|---|
| **BOT-1** | `@Android_opencode_bot` ↔ **Android device** opencode | **Target:** run `opencode-bot --id=android` **in proot on this phone**, workspace `/root/Health-tracker`, token from proot `.env` (`ANDROID_OPENCODE_BOT_TOKEN` / meal_audit `.env`). **Step A:** `sudo systemctl disable --now opencode-bot@android` on VPS (stop wrong owner). **Step B:** start device poller (nohup/tmux in proot). **Invariant:** one `getUpdates` per token — VPS and phone must not both poll. Registry entry `android` stays; only the **runtime host** changes. |
| **BOT-1b** | VPS android service (wrong host) | **OPEN — disable.** `opencode-bot@android` is active on VPS as of 2026-09-22 22:26 UTC; that association is temporary/mistaken relative to BOT-1. Disable before starting the phone poller. `@Opencode_135_bot` stays on VPS (correct). |
...
| **BOT-4** | Model A E2E from **device** OpenCode | Chat `@Android_opencode_bot` → must hit **phone** opencode/proot workspace → audit skill → `MEDIA:`. Prove after BOT-1 host flip. `@Opencode_135_bot` remains the VPS door. |
...
**Do not** dual-poll one Telegram token (VPS + phone, or Hermes + opencode-bot). **Do not** restart `hermes-gateway` from this track unless a named V-28/V-29 task says so. **Do not** leave `@Android_opencode_bot` permanently on the VPS if BOT-1 is the intent.
