...
**Track V Phase 10 pickup:** V-30.1 is DONE; V-30.2 code/fixtures are merged but its P9 token, registry handle, and live Telegram E2E are still missing. Do not call V-30.2 DONE or start V-30.3 until P9 is closed. The complete dependency order, exact runbook, and V-30.3–V-30.5 file/gate handoff are in `plan/BUG_TICKET_PIPELINE.md` §6.2.
...
### Bot side — OPEN (resumed 2026-09-24 — D-2 closed; pickup: AI_HANDOVER.md top entry, then BOT-12)
...
| **BOT-4** | Model A E2E from **device** OpenCode | **OPEN — human proof:** message `@Android_opencode_bot` → must answer via **phone** proot opencode (workspace `/root/Health-tracker`). Model now muse-spark-1.3 free (android override). **DONE 2026-09-22:** phone owns the token (offset 433856307 -> 433856314), live chat got a phone-opencode muse-free reply ~23:33, **user confirmed the answer quality**. `@Opencode_135_bot` remains the VPS door. |
...
| **BOT-12** | History parity P1 (2026-09-24): cline `--id` resume per chat into the same `sessions.json` contract + `reply_to_message` quote inclusion on all bot-host lanes. Files: `scripts/lib/agent-cline.mjs` (capture session id, accept resume id), `scripts/bot-host.mjs` (pass/save per-chat cline id; prepend quoted text). Prereq: one live probe to find where cline `--json` emits the session id. **Research constraint (2026-09-24): resume injects the compact *summary*, not the raw transcript — full-history replay measurably degrades (96% vs 71%, see AGENT_ALIGNMENT.md §5).** Gate: unit (resume args, quote prepend) + live: VM2 answers a follow-up referencing the prior turn. | **PARTIAL 2026-09-24** — shared `reply_to_message` text/caption quote prompt shipped for all bot-host lanes; 115/115 `tests/bot-host.test.ts`, tsc/syntax/Guard green. Cline resume BLOCKED: installed/upstream CLI 3.0.65 forces `--id` into interactive chat mode, clears the new prompt, and rejects `--json`; JSON stream exposes `taskId=conv_...`, not resumable root `sessionId`. Do not ship a fake `--id`; resume after upstream exposes root ID + headless follow-up. |
| **BOT-13** | Memory parity P2 (2026-09-24): repo-owned `bots/memory/<bot-id>.md` + shared `bots/memory/USER.md`, injected into every bot-host prompt (all lanes — prompt-level, zero runner changes), grown via `/remember <fact>` + `/compact` summary auto-append. Hermes keeps its store but reads the same `USER.md` (no fork). Gate: checker lists memory files; live: fact survives `/new` + restart. | **OPEN.** |
| **BOT-14** | Skills-bridge repair P3 (2026-09-24): the two dead `shared_skills` paths in `bots/registry.json` (`social-media/telegram-media-delivery`, `autonomous-ai-agents/opencode` — neither exists on disk) become symlinks/copies of `scripts/skills/common` via `sync-hermes-skills.sh` + a Mac-side parity check. Grok box picks up `tools/telegram-provider-router/docs/BOX_ALIGN_PLAN.md`. Gate: no registry skill path missing on disk. | **OPEN.** |
| **BOT-15** | Learning-loop unify P4 (2026-09-24): one loop — repo `failure-log.mjs` is the writer, `review-failures.mjs` output lands where every agent sees it (memory-file footer is the cheap option); Hermes `bot-failures.jsonl` becomes a reader. **Process (§4b): record on ALL lanes (today opencode-only) → weekly VPS+Mac log merge + review → every LEARN signature gets one durable artifact (test/rule/skill/soul-line) → artifacts in git so all hosts inherit. Verified gap: Mac log empty, review never scheduled.** **Research upgrade (2026-09-24): Memory-as-Governance gate — deterministic pre-action warning before repeating a logged failed fix or touching a known-fragile file (`file-locks` + `failure-log` are 80% there; the gate is the missing 20%).** Close `proc-learning` partial. Gate: 2nd same-signature failure yields sensor/standing update on any class. | **OPEN.** |
| **BOT-16** | Soul parity P5 (2026-09-24): **composed soul (§4a): `bots/soul.md` base + per-capability `soul` lines in `capabilities.json` (checker-verified) + `bots/soul.<id>.md` override** — one feature edit updates one line, all bots inherit; Hermes `SOUL.md` stays wording source. **Anti-spaghetti (§4a): namespaced `[shared:*]` / `[cap:<id>]` / `[bot:<id>]` containers, one owner per container, line budgets, contradiction check.** Gate: replies match length/weight rule on a probe set. | **OPEN.** |
| **BOT-18** | Auto-heal ladder (2026-09-24, §4c): **tracking first — `runs.jsonl` per-run ledger (open-at-start lease, close-at-finish; OPEN at boot = crash) + poll-loop heartbeat; crash/pending/error becomes a query.** L1 process (systemd `Restart=always` DONE; +409 loud exit) → L2 zero-LLM run watchdog in the progress event stream (stall timer, error-cascade counter, repeat fingerprint; nudge then abort-with-terminal-message) → L3 bounded recovery (retry/continue-with-summary/failover, max 2/turn, partials preserved) → L4 repair-on-demand (twice-flagged signature spawns one fix+critic investigatory run; output = patch or card; learning artifact closes the loop; config-repair auto, code-repair via review/CI, never autonomous commit). **Playbooks (§4c): A = stuck-long (kill-and-resume ladder, shape-keyed: repeater/wanderer/looper); B = crash-as-pending (run-lease + boot sweep edits orphaned ⏳ to terminal receipt + learns).** Literature: Samsung threshold framework, Cloudflare durable recovery, heartbeat plugin, SelfHeal, agentpatterns ladder, OTel-shaped local ledger. | **OPEN.** |
| **BOT-17** | Lane contract — bot = location, execution lane = tool, provider/model = backend (2026-09-24): `vm`/`mobile`/`collab` name a *place*; `opencode`/`cline`/`gemini` are execution surfaces selected per message via `parseModelRef`; Gemini and Token Harbor are provider/model choices behind a tool/runner, not independent bot identities. Work: (1) picker honesty — API-only backends declare no live tools/session; (2) registry separates location, execution surface, and default provider/model; (3) every BOT-12…16 contract holds on every supported execution surface or the surface is declared degraded; (4) the same work-session/debug contract applies across all backends. Gate: same probe prompt through each supported execution surface from one bot; only declared differences. | **OPEN.** |
| **BOT-19** | Tool-agnostic work-session observability and shared terminal visibility — create a session on demand per active location/chat/workspace, never permanently per bot. `tx on` enables the shared work view; `tx off` hides it without stopping work; `tx status` and `tx debug` inspect the active work session. Generic `/debug`, `/handoff`, and `/abort` work for every bot/backend; `tx` never owns stop/abort semantics. One physical location maps to one tmux session; each bot/workstream maps to a window. tmux is only the live-terminal adapter for TTY-capable execution surfaces; API-only providers expose structured events/transcripts and report live attach as unavailable. `/status` shows work-session, execution surface/provider, controller, and debug capability. Shared observation is default; input is serialized so bot and human do not interleave PTY writes. Lane changes stay in the same work session with handoff. Gate: all supported backends answer the same debug/status probe; terminal backend attaches and shows shared activity; API backend returns an honest event/debug view; handoff/abort preserve state; no raw secrets in Telegram. | **OPEN.** |
...
### Track V Phase 10 — Bug ticket pipeline for TG agentic development (V-30.1 DONE 2026-09-24 — V-30.2 code done, P9/E2E pending — V-30.3…V-30.5 NOT STARTED)
...
**Status:** **V-30.1 DONE 2026-09-24** (store + CLI + A-f1/A-f5; gates green). **V-30.2 CODE DONE 2026-09-24**, but **not operationally done**: P9 token is absent, `hermes_bug_ticket.enabled` is `false`, username is `null`, and no E2E reply is recorded. **V-30.3, V-30.4, and V-30.5 have not started** and require their own explicit human go after predecessor gates.
...
| **V-30.2** | Packer code is merged: `bug-pack.mjs`, `bugctl pack --check`/`--split`, Hermes profile/skill, registry entry, and capability rows. **Operational closure remains:** traceable BUG-8449 fixture/server contract, P9 token, BotFather handle, token sync, one live E2E reply, then `enabled: true`. | Fixtures: (a) BUG-8449's 7-item report → **1 card + a split list**, never a bundle; (b) vague → `needs_repro`; (c) duplicate → merged; plus live E2E before enabling. | **CODE DONE / OPERATIONS PENDING** — helper fixtures green but source/server parity still open; token absent, registry `enabled:false`, username `null`; see pipeline §6.2 |
| **V-30.3** | Reproducer on existing QA profiles: add `qa-reproduce`, `qa-runner.mjs --ticket=<#n>`, `bugctl repro --check`, R2 artifact keys, and verdict writes. No new QA bot/token. | Known-good posts `repro.status=failed` + `run.log` → derived `not_reproducible`; known-bad posts `confirmed` + command/exit/log/`before.png`; both verdicts persist. | **NOT STARTED** — blocked on V-30.2 P9/E2E and explicit human go; do not confuse `not_needed` with `not_reproducible` |
| **V-30.4** | Orchestrator: packet-driven `run-coding-dispatch.sh --ticket=#n`, plan block, start/end attempt rows, `blocked_reason`, idempotent in-flight guard, verifier separation. | Scratch dev-only card reaches `done` only after named gate green; failed dispatch leaves `blocked_reason`; no direct main push/chat-only claim. | **NOT STARTED** — after V-30.3 and explicit human go |
| **V-30.5** | Memory + ratchet: generated `bug-backlog.mjs`, `/resume`, add `svc-repro` without duplicating existing rows, status transitions from proof, CI gates, `docs/agent/BUG_PIPELINE.md`, `telegram_work.md`, and BUG-8449 retro-audit. | `review-failures` has no repeated fixture signature; global capability checker `--strict` exits 0; continuity/pack gates run in CI; retro-audit answers §4.10 questions 1–6 from disk/API only. | **NOT STARTED** — after V-30.4; strict is currently blocked by 5 pre-existing partial capability rows plus ticket rows not yet proven |
...
**Do not start** V-30.3…V-30.5 before an explicit human go. V-30.2 code + fixtures landed
2026-09-24 with `hermes_bug_ticket` **enabled: false**; the remaining V-30.2 step is the human
P9 token + one E2E reply, then flip `enabled: true`. V-30.3 must not begin until that closure is
recorded. V-30.5's global strict capability gate is also currently red: five unrelated rows are
`partial`; do not mark them `done` or weaken the checker.
