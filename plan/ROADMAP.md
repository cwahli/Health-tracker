...
**Next code is BOT-20.** Load [docs/agents/bot_work.md](../docs/agents/bot_work.md) and stop. Do not load the bot table below for execution. BOT-18 is DONE.
...
| **BOT-20** | Pack, then reject — next code. `run-coding-dispatch.sh` calls the existing `scripts/lib/bug-pack.mjs` (`packCheck`, `splitMultiItemReport`). Four fields (`page`, `observed`, `expected`, `screenshot`) or a versioned `work_item` in. Several defects become one card plus a split list. A payload that fails `packCheck` exits non-zero and is not passed through as `--task`. Do not write a second packer. | **OPEN — next code.** |
