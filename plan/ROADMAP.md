...
| Meal-Audit Bot (audit of live meal turns) | **DONE P0–P6** at `2b858ff` — plan [MEAL_AUDIT_BOT_PLAN.md](./MEAL_AUDIT_BOT_PLAN.md). Open for VM agent: VPS `$HOME/.hermes/profiles/meal_audit` provision + `hermes gateway` restart, live phone `MEDIA:` E2E (needs bot token), fix open ledger `MAI-20260922-001` (turnId mismatch), seed real comparison fixtures, re-run `scripts/meal-audit-suite.mjs calibrate`. |
