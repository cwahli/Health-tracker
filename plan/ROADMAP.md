...
|---|---|---|---|
| **F-12.1** | Remove `searchUSDA` / `fetchUSDAFoodById` / two-round helpers and all Analyze call sites | `rg searchUSDA` / `fetchUSDAFoodById` empty in `server*.ts` + `src/server/food`. `tsc` unchanged baseline | Wrap in a flag |
