...
**Blocked on R-14.1 completion** (or an explicit human reorder). This is the live scorecard matrix for the quota-resilience work the user ordered 2026-09-26. It does not replace R-14.1 cards 4/5/6/6b/6c/9, BOT-24, or R-15 — it is the pass/fail board they all report into. No R-16 row may start while R-14.1 is open, and no row reopens BOT-1–BOT-23, V-30, CB-6–CB-8, or a new bot per place/role/project.

**Shared-components law (no custom builds):** every row below ships through the canonical shared modules, never a per-bot copy. Canonical: `scripts/lib/free-lanes.mjs` (ledger, `stampDepleted`/`trackRunQuota`, usable-choice projection) + vendor mirror `tools/telegram-provider-router/src/free-lane-table.vendor.mjs` (via `node scripts/sync-router-vendor.mjs`, checked by `scripts/check-capability-propagation.mjs`); `scripts/lib/lane-contract.mjs` (`laneSupports`); Grok-built failover behavior `markDepleted` / `isDepleted` / `nextFailoverRoutes` (+ `FREE_ALLOWANCE_BUCKETS`, shared vs per-model buckets); `scripts/lib/freemodels.mjs` + `scripts/tool-allowance.mjs` / `scripts/lib/tool-allowance-ping.mjs` (host capability probe); catalogs `golden/scorecard/current/FREE_CODING_TOOLS_CATALOG.md` + `FREE_MODEL_TOOL_PICKER.md` + `FREE_MODEL_BAKEOFF.md` (tiers + benchmark ledger). A fix that lands in only one bot or only one surface is a fail for that row.

**Tier + catalog law (follow existing work, do not reinvent):** high (coding-agent-capable) vs light (docs/inventory) split follows `FREE_CODING_TOOLS_CATALOG.md` Ranked picks + Tools × free models and `FREE_MODEL_TOOL_PICKER.md` defaults (today: high = DeepSeek V4.1 Flash / Muse Spark 1.3 Contributor-class; light = Laguna S 2.1-class and equivalents; Freebuff/Token Harbor/Gemini-through-OpenCode placed by the same tables). Benchmark score shown on `/freemodel` comes from `FREE_MODEL_BAKEOFF.md`, not a new rating. `/model` is the live setter; `/freemodel` is the free-lane picker; `/allowance` is the ledger board — all three read the same per-worker ledger.
...
|---|---|---|---|
| **QS-1** | Live agent roams mobile → VM → grok → collab within one project + CLI (OpenCode, Cline, etc.) | Same project (e.g. `/project 1` and one `external-N`), one chat runs one turn on each of `vps`/`mobile`/`grok`/`collab` via `/location <host>` on OpenCode and on Cline where that host has it; each turn's ledger stamp lands on the host that ran it (VM ledger mtime untouched by a mobile run and vice versa); unreachable host warns and does not run on the VM (card-4 pattern) | 4 location replies + per-host ledger stamps/mtimes + `BOT_LOCATION` in service environ; unreachable-host negative check |
| **QS-2** | Live resilience against exhausted quota | Quota-shaped 429/limit error on the first selectable lane never surfaces raw JSON (the 2026-09-25 12:29Z `INFERENCE_CAP_ERROR` reply is the fail specimen); the same prompt completes on the next lane with a user-visible switch line naming failed lane → next lane | Before/after `/allowance`, raw reply naming next lane, completed answer on next lane |
| **QS-3** | Exhausted host hands off project + full context to a location with quota | When a worker's selectable list empties, the next connected worker continues with the same project, same role, and the handoff pack; `/project` state and role survive the hop; empty-everywhere replies "no location has quota" and does not call the failed lane | Reply naming next host + project/role check on the new host + ledger copies from both hosts |
| **QS-4** | Automated pack-up with three paths | Handoff pack is built from files on disk with no model call (project, role, `index.md`, turn log); if the log is long and another connected lane has allowance it may write the short summary; if little info, auto-pack with no summary call; the reply states which path wrote it (disk-pack / lane X wrote summary / summary skipped, disk pack sent); the just-failed lane is never asked to summarize itself | The pack (or its manifest) + reply line stating the path + proof the failed lane made no model call |
| **QS-5** | Every agent has the correct `/model` + `/freemodel` + `/allowance` for its location | Per location, a probe table is captured (installed tools, local OpenCode/Cline binary + auth, OpenCode provider catalog, Token Harbor credential/catalog, Freebuff binary/credential, Gemini-through-OpenCode) and the `/freemodel` + `/allowance` rows for that host match exactly what that probe supports — nothing inferred from another host; `/model` lists/sets the free model for the active provider on that worker | `node scripts/probe-free-lanes.mjs` zero-burn output per host + `/freemodel` + `/allowance` screenshots/text per host/bot showing the match |
| **QS-6** | Free list split high (coding-capable) vs light, following existing work | `/freemodel` renders the two tiers from the catalogs above (high first, light second, terminal-only/degraded marked); no new tier file, no per-bot tier fork; Cline-on-phone and Freebuff-Telegram-style impossibilities stay honestly degraded, never selectable | Tiered `/freemodel` render per host + catalog row pointers proving the split was inherited |
| **QS-7** | Benchmark score shown on `/freemodel` selection | Each selectable `/freemodel` row carries its bakeoff benchmark score/label from `FREE_MODEL_BAKEOFF.md`; a model with no bakeoff entry shows "unranked", never an invented number | `/freemodel` render showing scores + bakeoff ledger pointers |
| **QS-8** | `/freemodel` list == `/allowance` list (same rows, same keys) | For the same host + bot, the normalized route-key set rendered by `/freemodel` equals the usable-row set in `/allowance` (same canonical projection; depleted rows show reset time in both; `ended`/unauthenticated/pending rows selectable in neither; no double-list, no label-in-one-surface-only) | Side-by-side `/freemodel` + `/allowance` captures per host/bot + projection pointer (`free-lanes.mjs` shared export) |
| **QS-9** | All agents auto-switch location (mobile ↔ VM ↔ grok ↔ collab) on exhaustion without failure | Depleting every selectable lane on the starting host auto-continues the same chat on the next connected host with quota (order = preference/ledger order); no hang, no manual `/location`, no dropped turn; each hop names the new host | Full-drain transcript: per-hop replies + ledgers proving each host emptied in order |
| **QS-10** | Exhaustion walks within high/light tier first, then next location's allowance | Failover order per worker: next selectable row in the same tier → remaining tier on the same worker (high exhausts before light is used as fallback, or vice versa per ledger preference — but tier-internal first) → next connected worker's walk; Freebuff never chosen; `ended` never offered; shared-bucket (e.g. OpenCode Zen free) marks all siblings depleted together | Ordered failover transcript + `/allowance` showing sibling-bucket depletion + proof Freebuff/`ended` skipped |
| **QS-11** | Mid-turn exhaustion auto-detected, flagged, allowance updated, next model resumes | A model that dies mid-stream with a quota signal is detected by the error parser (not by the user re-sending); the lane is stamped `depletedUntil` (+ reset hint) before the reply; `/allowance` reflects it on the next read; the turn resumes on the next available model (same location first, then another location) with a flag line; second identical signature appends one dead-end row per the learning loop | Mid-turn transcript with flag/switch lines + pre/post `/allowance` + dead-end row on repeat signature |
| **QS-12** | No custom builds — shared components only | `check-capability-propagation` green (vendor mirror byte-identical); bot-host, collab, mobile, and Grok router `/freemodel` + `/allowance` + failover all consume the same canonical projection/ledger helpers; a per-bot or per-surface fork of lane choice, stamping, or rendering fails this row even if the demo passes | Gate output + code pointers showing shared import on every surface + negative check (no surface-local lane table) |

**Gate (in order, then live):** `node scripts/check-capability-propagation.mjs` · `node scripts/probe-free-lanes.mjs` (zero-burn per host) · the BOT-24 VM + mobile live checklist · R-14.1 cards 4/5/6/6b/6c evidence blocks · then the QS-1…QS-12 matrix above in one pass. Mark R-16 DONE only when all twelve rows are green with pasted live evidence; otherwise re-iterate the red rows.

**Do not:** burn real free allowance for proof; hand-edit a ledger stamp; dual-poll a token; move a poller between machines; install Antigravity; add a bot per role/project/place; blank `bots/soul.md`; treat `/model` as the catalog (it is the setter — the catalog lives in `FREE_CODING_TOOLS_CATALOG.md`); show a benchmark number with no bakeoff row; offer Freebuff/`ended`/unauthenticated lanes as selectable; paste raw provider JSON to the chat as the "switch".
...
**Recorded gap, not the current job.** Do not start these packets while R-14.1 is open. An agent that reads this section and implements the projection is on the wrong task. The 2026-09-25 12:29Z VM-bot failure is the specimen for R-14.1 card 6: `cline-free/muse-spark-1.3-contributor` returned `429` with `Try again in 22h 46m`, and the chat received the raw `INFERENCE_CAP_ERROR` JSON. The lane was not stamped and the next lane was not selected.
