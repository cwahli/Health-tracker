...
| **BOT-16** | Soul parity P5 (2026-09-24): **composed soul (§4a): `bots/soul.md` base + per-capability `soul` lines in `capabilities.json` (checker-verified) + `bots/soul.<id>.md` override** — one feature edit updates one line, all bots inherit; Hermes `SOUL.md` stays wording source. Stops verbose drift, makes tone testable. Gate: replies match length/weight rule on a probe set. | **OPEN.** |
| **BOT-18** | Auto-heal ladder (2026-09-24, §4c): L1 process (systemd `Restart=always` DONE; +409 loud exit) → L2 zero-LLM run watchdog in the progress event stream (stall timer, error-cascade counter, repeat fingerprint; nudge then abort-with-terminal-message) → L3 bounded recovery (retry/continue-with-summary/failover, max 2/turn, partials preserved) → L4 repair-on-demand (twice-flagged signature spawns one fix+critic investigatory run; output = patch or card; learning artifact closes the loop). Literature: Samsung threshold framework, Cloudflare durable recovery, heartbeat plugin, SelfHeal. | **OPEN.** |
...
| **P4** | Track numbering | V-30.x (Track V Phase 10, this section) · a BOT-xx range in the bot table | V-30.x — **BOT-12…17 are all taken** as of 2026-09-24 (BOT-12…16 = Hermes-parity P1–P5, commits `17873f2`/`fbe0405`; BOT-17 = lane contract, `fdbf05d`), so that range is not available | docs only |
