...
| **BOT-12** | History parity P1 (2026-09-24): cline `--id` resume per chat into the same `sessions.json` contract + `reply_to_message` quote inclusion on all bot-host lanes. Files: `scripts/lib/agent-cline.mjs` (capture session id, accept resume id), `scripts/bot-host.mjs` (pass/save per-chat cline id; prepend quoted text). Prereq: one live probe to find where cline `--json` emits the session id. **Research constraint (2026-09-24): resume injects the compact *summary*, not the raw transcript — full-history replay measurably degrades (96% vs 71%, see AGENT_ALIGNMENT.md §4).** Gate: unit (resume args, quote prepend) + live: VM2 answers a follow-up referencing the prior turn. | **OPEN — proposed next.** |
...
| **BOT-15** | Learning-loop unify P4 (2026-09-24): one loop — repo `failure-log.mjs` is the writer, `review-failures.mjs` output lands where every agent sees it (memory-file footer is the cheap option); Hermes `bot-failures.jsonl` becomes a reader. **Research upgrade (2026-09-24): Memory-as-Governance gate — deterministic pre-action warning before repeating a logged failed fix or touching a known-fragile file (`file-locks` + `failure-log` are 80% there; the gate is the missing 20%).** Close `proc-learning` partial. Gate: 2nd same-signature failure yields sensor/standing update on any class. | **OPEN.** |
| **BOT-16** | Soul parity P5 (2026-09-24): one short `bots/soul.md` (per-id override allowed), injected like memory; stops verbose drift, makes tone testable. Hermes `SOUL.md` stays source for wording. Gate: replies match length/weight rule on a probe set. | **OPEN.** |
...
| | Full matrix + target architecture: `plan/AGENT_ALIGNMENT.md` (audited 2026-09-24, prior art §4). Rule for BOT-12…17: shared change in `scripts/lib` or prompt-level — never one-agent patches. | |
...
| ID | Phase | Done when | Status |
|---|---|---|---|
| **V-30.0** | **Decide the pending actions below** and record each answer in §8 of [BUG_TICKET_PIPELINE.md](./BUG_TICKET_PIPELINE.md). | P1–P10 each have a chosen option — no `recommended` left unanswered. | **PENDING HUMAN** |
...
**V-30.0 pending actions — decide each one, then record the answer in §8 of [BUG_TICKET_PIPELINE.md](./BUG_TICKET_PIPELINE.md):**
...
| **P4** | Track numbering | V-30.x (Track V Phase 10, this section) · BOT-12…16 in the bot table | V-30.x — **do not use BOT-12…16**: `plan/AGENT_ALIGNMENT.md` already cites BOT-12 (memory injection) and BOT-15 (governance gate) for its P1–P5 rails | docs only |
