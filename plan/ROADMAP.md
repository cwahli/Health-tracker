...
**Next code is BOT-18.** Load [docs/agents/bot_work.md](../docs/agents/bot_work.md) and stop. Do not load the bot table below for execution.
...
**Bot code** follows the rail above (BOT-18, then BOT-20 using the existing `scripts/lib/bug-pack.mjs`). **Track V Phase 10 pickup:** V-30.1 is DONE; V-30.2 code/fixtures are merged but its P9 token, registry handle, and live Telegram E2E are still missing. Do not call V-30.2 DONE or start V-30.3 until P9 is closed. The complete dependency order, exact runbook, and V-30.3–V-30.5 file/gate handoff are in `plan/BUG_TICKET_PIPELINE.md` §6.2.
...
**Cold agent:** load `docs/agents/bot_work.md` only. This table is reference. BOT-1–BOT-8 are history. There is no Cline agent. The Cline CLI adapter is degraded for session resume and is not the pickup. V-30.2 code is merged; its remaining work is the human P9 token in `plan/BUG_TICKET_PIPELINE.md` §6.2.
...
| **BOT-13** | Retrieved memory. Stores `decisions/`, `dead-ends/`, `facts/`, plus shared `USER.md` under the existing caps. Retrieve only on a build, investigate, or decide turn. Do not inject a whole file. Do not append a `/compact` summary. Missing, over-cap, or stale → a receipt. Count false-fires. | **OPEN.** After BOT-14. |
...
| **BOT-15** | One outcome row per dispatch: ticket, surface, provider/model, defect class, tokens, wall-clock, outcome. The pre-action gate reads it in code, beside the file locks, before the edit. The second identical signature writes one test or rule the same day. A prompt footer is not the gate. Hermes `bot-failures.jsonl` is a reader. | **OPEN.** After BOT-13. |
| **BOT-16** | Composed soul: `bots/soul.md` base + one `soul` line per capability + `bots/soul.<id>.md` override. Line budgets. The three laws in `docs/agents/bot_work.md` (one defect, the script starts the coder, `named_test` closes the card) live in the shared soul, because the Hermes gateway cwd is `~/.hermes` and a Telegram turn does not load `AGENTS.md`. | **OPEN.** After BOT-15. |
| **BOT-18** | Silence to a receipt — next code. Write `{chatId, messageId, startedAt, pid}` at run start. On boot, edit every orphaned progress message to `restarted mid-run — send it again` and append a `crash-pending` row. On Telegram 409, `process.exit(1)` (today the process logs the risk and keeps polling). Later rungs (watchdog, resume, code repair) stay parked. Config repair only. No autonomous commit. | **OPEN — next code.** |
| **BOT-17** | Agnostic lane contract. A bot id is a place. The runner for a message is a surface, not an agent. A model is a backend. There is no Cline agent and no Gemini agent. An API-only backend declares no tools and no session. A surface that cannot honor session, memory, the typed ticket, or the crash receipt is marked degraded (the Cline adapter already is, for resume). | **OPEN.** |
...
| **BOT-20** | Pack, then reject. `run-coding-dispatch.sh` calls the existing `scripts/lib/bug-pack.mjs` (`packCheck`, `splitMultiItemReport`). Four fields (`page`, `observed`, `expected`, `screenshot`) or a versioned `work_item` in. Several defects become one card plus a split list. A payload that fails `packCheck` exits non-zero and is not passed through as `--task`. Do not write a second packer. | **OPEN.** After BOT-18. |
| **BOT-21** | Coordination-tax log. `dispatch_lock` stops two writers. Log `(ticket, agent, tool, args-hash)`. The same hash twice on one ticket alerts. Two repro verdicts on one card must match or escalate. | **OPEN.** After BOT-20. |
| **BOT-22** | Grok router session split. `tools/telegram-provider-router` keeps one global session for every chat. Move it to per-chat. Same contract as BOT-12. | **OPEN.** After BOT-16. |
