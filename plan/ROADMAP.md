...
| **BOT-10** | Bot succession (2026-09-24): `@Opencode_135_bot` → `@VM_19485_bot` (**LIVE** — `vm` already answers as `@VM_19485_bot`); `@Android_opencode_bot` → `@mobile_8768_bot` (registry `mobile` entry added, `MOBILE_BOT_TOKEN` in master tokens file, phone-side E2E pending); new `@VM2_19485_bot` (registry `vm2` exists but **blocked — `VM2_BOT_TOKEN` missing from master `tokens.env`**). Old bots stay until every feature is replaced, then removed. **Location rule — host follows the name:** `VM*` bots run on the VPS (`bot-host` systemd), `mobile*` on the phone (Termux/proot), `collab` on Colab, `hermes*` on the Hermes gateway. Never cross-host a token (BOT-5 one-poller law still applies). Token flow documented in `bots/TOKENS.md` (master → `sync-bot-tokens.mjs` → per-runtime env). | **PARTIAL** — to finish: add `VM2_BOT_TOKEN=` to master tokens file, run sync, enable + start `bot-host@vm2`, prove one E2E reply per new bot, then retire old handles. |
