...
| **BOT-12** | History parity P1 (2026-09-24): cline `--id` resume per chat into the same `sessions.json` contract + `reply_to_message` quote inclusion on all bot-host lanes. Files: `scripts/lib/agent-cline.mjs` (capture session id, accept resume id), `scripts/bot-host.mjs` (pass/save per-chat cline id; prepend quoted text). Prereq: one live probe to find where cline `--json` emits the session id. **Research constraint (2026-09-24): resume injects the compact *summary*, not the raw transcript — full-history replay measurably degrades (96% vs 71%, see AGENT_ALIGNMENT.md §5).** Gate: unit (resume args, quote prepend) + live: VM2 answers a follow-up referencing the prior turn. | **OPEN — proposed next.** |
...
| **BOT-17** | Lane contract — bot = location, lane = tool (2026-09-24): `vm`/`mobile`/`collab` name a *place*; `opencode`/`cline`/`gemini` is a per-message tool choice (`parseModelRef`), never an identity. Work: (1) picker honesty — gemini entries already labelled `(api)`; add a no-tools/no-session warning when a gemini lane is selected; (2) registry separates location from default tool (no per-bot lane exceptions); (3) every BOT-12…16 contract holds on all three lanes or the lane is declared degraded. Gate: same probe prompt on all three lanes from one bot; only declared differences. | **OPEN.** |
| | Full matrix + target architecture: `plan/AGENT_ALIGNMENT.md` (audited 2026-09-24, prior art §5, three systems §4). Rule for BOT-12…18: shared change in `scripts/lib` or prompt-level — never one-agent patches. | |
...
### Track V Phase 10 — Bug ticket pipeline for TG agentic development (PROPOSAL — BLOCKED on V-30.0)
...
**Status:** PROPOSAL ONLY — **no code, no token, no `src/` change.** The deliverable so far is the plan document. Phase 10 does not start until every pending action in V-30.0 has a human answer.
...
| **P2** | **State vocabulary** | S-C derived, full 11 names (precedence list in §4.3.4) · S-C-lite, 5 names + flags (`new/packed/in_fix/verifying/done` + `needs_repro`/`not_reproducible`/`blocked_reason`/`duplicate_of`, §4.3.5) · S-C-lite without `not_reproducible` (folded into `blocked_reason`) | S-C-lite | V-30.1 |
...
| **P9** | Human: create the packer token | BotFather → `HERMES_BUG_TICKET_TOKEN` → master `tokens.env` → `sync-bot-tokens.mjs` | required before the V-30.2 E2E proof | V-30.2 |
...
**Do not start** V-30.1 — and do not create the `bug_ticket` profile or its token — while any
pending action above is unanswered. Until then the only deliverable is the plan document.
