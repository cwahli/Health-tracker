...
### Track V Phase 10 — Bug ticket pipeline for TG agentic development (V-30.1 DONE 2026-09-24 — V-30.2 packer shipped pending P9 token E2E — V-30.3…V-30.5 gated on explicit human go)
...
**Status:** **V-30.1 DONE 2026-09-24** (store + CLI + A-f1/A-f5; gates green). **V-30.2 packer CODE DONE 2026-09-24** (`bugctl pack --check`, skill, profile, registry `enabled:false`, fixtures a/b/c green) — **blocked on human P9** (BotFather `HERMES_BUG_TICKET_TOKEN` → master `tokens.env` → `sync-bot-tokens.mjs` → one E2E reply → then flip `enabled: true`). V-30.0 decisions all recorded (§6.1/§8). V-30.3…V-30.5 start only on an explicit human go; phases run in order.
...
| **V-30.2** | Packer: Hermes profile `bug_ticket` — BotFather token → master `~/.config/bot-host/tokens.env` → `sync-bot-tokens.mjs` → registry entry `hermes_bug_ticket` → short soul (the 3 laws only, `orchestrator-dispatcher` **not** linked) + `scripts/skills/common/bug-ticket/SKILL.md` (synced by `sync-hermes-skills.sh`; `.agents/skills/` is journey skills, not Hermes) + `bugctl pack --check` + capability rows `svc-bug-ticket`/`proc-ticket-state`/`sess-ticket-resume`. | Fixtures: (a) BUG-8449's 7-item report packs to **1 card + a split list**, never a bundle; (b) a vague report → `needs_repro`; (c) a duplicate → merged; plus one E2E reply through the new token before `enabled: true`. | **CODE DONE 2026-09-24** — fixtures green (`assert-bug-pack` 31/31, vitest 35, continuity 33, lint 0); **P9 token E2E still human** before `enabled: true` |
| **V-30.3** | Reproducer on the **existing** QA profiles: new `scripts/skills/common/qa-reproduce/SKILL.md`, `qa-runner.mjs --ticket=<#n>`, repro verdict writes. No new QA bot, no new token. | A known-good card reports `not_reproducible`; a known-bad card reports `reproduced` with `run.log` + `before.png` artifact keys. | **After V-30.2** (P7 = wake QA profiles on demand) |
| **V-30.4** | Orchestrator: `run-coding-dispatch.sh --ticket=#n` (prompt built from the packet; `--task=` kept legacy), attempt rows at start and end, `blocked_reason` instead of a bare `escalated_human`, plan block on the card, verify closes it. | End-to-end on a scratch fixture bug → card `done` with the named gate green and **no** chat claim; a failed run lands as `blocked`. | **After V-30.3** |
| **V-30.5** | Memory + ratchet: generated `scripts/bug-backlog.mjs` (replaces the hand-compiled file), `/resume`, capability rows (`svc-bug-ticket`, `svc-repro`, `proc-ticket-state`, `sess-ticket-resume`) + `check-capability-propagation.mjs --strict` in CI, and the BUG-8449 retro-audit. | Continuity gate green in CI; retro-audit answers the 6 continuity questions for #8449 from disk alone (no chat log). | **After V-30.4** |
...
| **P5** | Owner of `not_reproducible` | packer decides close vs `needs_info` · orchestrator triages it | packer (orchestrator must not re-open the free-text hop) | V-30.2 |
...
P9 token + one E2E reply, then flip `enabled: true`. V-30.1 is DONE 2026-09-24.
