...
| ID | Item | Status / next |
|---|---|---|
| **BOT-1** | `@Android_opencode_bot` answers as opencode | Registry id `android` + `ANDROID_OPENCODE_BOT_TOKEN` + `systemctl enable --now opencode-bot@android` on VPS. Token lives in proot meal_audit `.env` **and** must be copied to `~/.config/opencode-bot/android.env` (no second poller on same token). |
...
| **BOT-3** | VPS `sync-hermes-skills.sh` | Re-run on VPS so meal_audit drops stale `orchestrator-dispatcher` symlink and gains `telegram-testing`. Proot mirror already synced. |
| **BOT-4** | Model A E2E from OpenCode chat | Ask `@Opencode_135_bot` (or android bot) to audit a meal → skill loads → entry command → `MEDIA:` on phone. Needs a running opencode-bot instance. |
| **BOT-5** | One-poller/token guard | Test: registry `tokenEnv`s must not equal Hermes profile `TELEGRAM_BOT_TOKEN`s (except intentional single owner). |
| **BOT-6** | `run-coding-dispatch.sh` `meal_audit` category | Optional: `qa_profile_for_category` branch so tickets can land in meal_audit chat — only if desired; default remains `qa_meal`. |
...
**Do not** dual-poll one Telegram token (Hermes gateway + opencode-bot). **Do not** restart `hermes-gateway` from this track unless a named V-28/V-29 task says so.
