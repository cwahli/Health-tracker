...
**Next code is BOT-21.** Load [docs/agents/bot_work.md](../docs/agents/bot_work.md) and stop. Do not load the bot table below for execution. BOT-20 is DONE.
...
| **BOT-21** | Coordination-tax log. `dispatch_lock` stops two writers. Log `(ticket, agent, tool, args-hash)`. The same hash twice on one ticket alerts. Two repro verdicts on one card must match or escalate. | **OPEN — next code.** |
