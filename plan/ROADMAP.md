...
| **BOT-1b** | VPS android service (wrong host) | **DONE — disabled.** `systemctl disable --now opencode-bot@android` on VPS. `@Opencode_135_bot` stays on VPS (correct). One `getUpdates` per token. **Proof 2026-09-22:** VPS journal `[android] connected as @Android_opencode_bot` 22:26 + VPS opencode ran `opencode-go/deepseek-v4.1-flash` 22:42-22:45 — the VM answered the early messages (VPS offset 433856307). Service now **masked** + inactive; only `--id=opencode` runs on VPS. |
...
| **BOT-4** | Model A E2E from **device** OpenCode | **OPEN — human proof:** message `@Android_opencode_bot` → must answer via **phone** proot opencode (workspace `/root/Health-tracker`). Model now muse-spark-1.3 free (android override). **DONE (technical) 2026-09-22:** phone owns the token (offset 433856307 -> 433856314), live chat got a phone-opencode muse-free reply ~23:33. User to confirm reply quality on wake. `@Opencode_135_bot` remains the VPS door. |
