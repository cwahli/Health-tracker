...
**Binding correction (2026-09-22, mid-course):** `@Android_opencode_bot` must talk to **this Android device’s opencode** (Termux / proot / the interactive phone session) — **not** the VPS. Name = device. The first direct link (token + chat `6218257274` in proot `~/.hermes/profiles/meal_audit/.env`, test reply `opencode online`) was the device path. Enabling `opencode-bot@android` **on the VPS** was wrong for that intent: it bound the bot to `/home/ubuntu/src/Health-tracker` + VPS opencode. **Change the bot mid-course = flip ownership to the phone and stop the VPS poller.**

| ID | Item | Status / next |
|---|---|---|
| **BOT-1** | `@Android_opencode_bot` ↔ **Android device** opencode | **DONE 2026-09-22 (device).** Registry `android`: workspace `/root/Health-tracker`, `opencodeBin` `/root/.opencode/bin/opencode`, skills `.agents/skills` + `/root/Health-tracker/scripts/skills`. Token: `/root/.config/opencode-bot/android.env` (`ANDROID_OPENCODE_BOT_TOKEN`). Runner: `~/start-android-opencode-bot.sh` (setsid/nohup restart loop; log `~/android-opencode-bot.log`). **Do not** start this id on the VPS. **Auth 2026-09-22:** phone needs `/root/.local/share/opencode/auth.json` (copied from VPS) or `opencode-go/deepseek-v4.1-flash` fails with `Unexpected server error` (no `OPENCODE_API_KEY`). Bot `--dry-run` PASS after auth copy. |
| **BOT-1b** | VPS android service (wrong host) | **DONE — disabled.** `systemctl disable --now opencode-bot@android` on VPS. `@Opencode_135_bot` stays on VPS (correct). One `getUpdates` per token. |
| **BOT-1c** | proot long-poll reliability | **DONE in `scripts/lib/tg-api.mjs`:** `AbortSignal.timeout` (poll+10s) so a dead Termux socket cannot hang forever. Phone network is still flaky (`fetch failed` / abort) — restart loop recovers; do not stack a second poller. |
| **BOT-2** | Missing `sharedSkills` packs | Dropped dead paths `~/.hermes/shared_skills/{social-media/telegram-media-delivery,autonomous-ai-agents/opencode}` from registry (absent on box). Re-add only after creating real `SKILL.md` packs under `~/.hermes/shared_skills/`. |
...
| **BOT-4** | Model A E2E from **device** OpenCode | **OPEN — human proof:** message `@Android_opencode_bot` → must answer via **phone** proot opencode (workspace `/root/Health-tracker`). Model/skills from registry master; phone `auth.json` present (deepseek works). Bot path `--dry-run` PASS 2026-09-22. Need live Telegram message from human. `@Opencode_135_bot` remains the VPS door. |
...
| **BOT-7** | Stale BOT_ROLES facts | Global `MEMORY.md` over cap / still mentions Render+no Orchestrator; QA souls still have `<Visual fix needed>` placeholders; `system_prompt_suffix` cleanup — V-28 leftovers, not meal-audit. |
| **BOT-8** | Meal-audit OPEN items (from live handoff) | Canonical bundle output dir; `MEDIA:` fence hardening; holdout→golden promote; suite scanner `holdout/`; W2 live run; document BotFather token path. See meal-audit row above. |

**Do not** dual-poll one Telegram token (VPS + phone, or Hermes + opencode-bot). **Do not** restart `hermes-gateway` from this track unless a named V-28/V-29 task says so. **Do not** run `opencode-bot@android` on the VPS. Restart phone bot: `setsid nohup ~/start-android-opencode-bot.sh </dev/null >/dev/null 2>&1 &`.
