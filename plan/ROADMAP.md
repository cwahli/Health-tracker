...
| **BOT-12** | History parity P1 (2026-09-24): cline `--id` resume per chat into the same `sessions.json` contract + `reply_to_message` quote inclusion on all bot-host lanes. Files: `scripts/lib/agent-cline.mjs` (capture session id, accept resume id), `scripts/bot-host.mjs` (pass/save per-chat cline id; prepend quoted text). Prereq: one live probe to find where cline `--json` emits the session id. Gate: unit (resume args, quote prepend) + live: VM2 answers a follow-up referencing the prior turn. | **OPEN — proposed next.** |
...
| **BOT-15** | Learning-loop unify P4 (2026-09-24): one loop — repo `failure-log.mjs` is the writer, `review-failures.mjs` output lands where every agent sees it (memory-file footer is the cheap option); Hermes `bot-failures.jsonl` becomes a reader. Close `proc-learning` partial. Gate: 2nd same-signature failure yields sensor/standing update on any class. | **OPEN.** |
...
| | Full matrix + target architecture: `plan/AGENT_ALIGNMENT.md` (audited 2026-09-24). Rule for all five: shared change in `scripts/lib` or prompt-level — never one-agent patches. | |
