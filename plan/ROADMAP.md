...
| **BOT-4** | Model A E2E from **device** OpenCode | **OPEN — human proof:** message `@Android_opencode_bot` → must answer via **phone** proot opencode (workspace `/root/Health-tracker`). Expect model/skills from registry master. `@Opencode_135_bot` remains the VPS door. |
