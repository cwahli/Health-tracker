...
**Bot code** follows the rail above (BOT-20 using the existing `scripts/lib/bug-pack.mjs`). **Track V Phase 10 pickup:** V-30.1 DONE, **V-30.2 CLOSED 2026-09-24** (P9 token, one live E2E reply, `enabled: true` + `@Bug_ticket_bot`, traceable fixture + packer-only boundary), **V-30.3 DONE 2026-09-24** (human go received; skill/runner/`repro --check`/gate `assert-bug-repro` 57/0 + live verdict on card #2 → `failed`/`not_reproducible`, `svc-repro` done), **V-30.4 CODE-DONE 2026-09-24** (human go received; `--ticket=#n` packet dispatch + guard + plan/attempt/block + verifier separation; gate `assert-bug-dispatch` 49/0; live scratch proof pending). V-30.5 remains gated on its explicit human go. The complete dependency order, exact runbook, and file/gate handoff are in `plan/BUG_TICKET_PIPELINE.md` §6.2.
...
### Track V Phase 10 — Bug ticket pipeline for TG agentic development (V-30.1 DONE · V-30.2 DONE 2026-09-24 · V-30.3 DONE 2026-09-24 · V-30.4 CODE-DONE 2026-09-24 — V-30.5 NOT STARTED)
