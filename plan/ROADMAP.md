...
| Meal-Audit Bot (audit of live meal turns) | **DONE P0–P6** at `2b858ff` — plan [MEAL_AUDIT_BOT_PLAN.md](./MEAL_AUDIT_BOT_PLAN.md). **VPS handoff 2026-09-22:** `meal_audit` profile provisioned (SOUL/config/preload `meal-audit-engine`, skills → src), gateway restarted (multiplexed), `MAI-20260922-001` verified fixed (MT/ST self-compare PASS), fixtures seeded `scripts/fixtures/meal_audits/`, suite report 2/2 PASS, calibrate score=100. **Still human:** dedicated `TELEGRAM_BOT_TOKEN` in `~/.hermes/profiles/meal_audit/.env` then live phone `MEDIA:` E2E. |
