...
| **BOT-10** | Bot succession (2026-09-24): `@Opencode_135_bot` → `@VM_19485_bot` (**LIVE**); new `@VM2_19485_bot` (**LIVE** 2026-09-24 — `vm2.env` synced from master, enabled, `bot-host@vm2` connected, 14 commands); `@Android_opencode_bot` → `@mobile_8768_bot` (registry `mobile` entry + `MOBILE_BOT_TOKEN` in master done — **phone-side left**: paste token on device, start phone poller, prove E2E, then retire `@Android_opencode_bot`). Old VPS bots stay until every feature is replaced, then removed. **Location rule — host follows the name:** `VM*` bots run on the VPS (`bot-host` systemd), `mobile*` on the phone (Termux/proot), `collab` on Colab, `hermes*` on the Hermes gateway. Never cross-host a token (BOT-5 one-poller law still applies). Token flow documented in `bots/TOKENS.md` (master → `sync-bot-tokens.mjs` → per-runtime env). | **PARTIAL** — remaining: phone E2E for `@mobile_8768_bot`, human E2E proof per VPS bot, retire old handles after parity. |
