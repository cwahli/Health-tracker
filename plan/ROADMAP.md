...
### BOT-19 live-observer pickup — REOPENED 2026-09-24

**Live failure proof.** On the VPS, VM `/tx on` created `work-vps:ws-vps-6218257274-home-ubuntu-s-fd07e7bc`, but the pane command was `bash` and showed only a prompt. The VM agent still answered and used tools through `runOpencode`; that process is spawned outside tmux with piped stdout. The current probe reports a window as “attached,” not a live observer. This is the defect: session creation shipped, output binding did not.

**Binding architecture — do not launch a second agent.** Keep the existing OpenCode/Cline child, JSON parser, Telegram progress, failover, and `/abort` ownership unchanged. Fan out the already normalized `onEvent` stream to a display-safe observer:

1. Write only allowlisted activity metadata: run start/complete, `thinking`, tool name + status, step token/cost totals, and aborted/failed. Never persist prompts, reasoning text, final text, tool input/output, raw errors, stderr, environment values, or file contents.
2. Store one private append-only log per hashed `(location, chat, workspace)` under `~/.hermes/work-observers/`; directory `0700`, file `0600`, bounded rotation, and no raw session id in the filename.
3. New work windows run `/usr/bin/tail -n 40 -F -- <observer-log>`. For the currently deployed legacy blank window, repeated `/tx on` must add a non-destructive observer pane and select it; never `send-keys`, `respawn-pane -k`, `kill-window`, or `kill-session` on an occupied/shared target.
4. Identify/reuse the exact observer pane by its expected tail command/log path. `/tx off` may stop only that verified observer pane; `/abort` still kills only the running agent child. Observer PIDs never enter the bot `running` map.
5. Preserve renderer-first event ordering and attach model context at each failover attempt. Observer failures are best-effort and cannot break Telegram delivery or the agent run.
6. Replace the misleading `Terminal: attached` wording with an observer-liveness probe. API-only lanes continue to report no live attach.

**Required sensors.** Unit coverage for secret-free formatting and private paths; fake-tmux coverage for new-window, legacy blank-window migration, idempotent reuse, exact-pane cleanup, and no destructive commands; event-fanout coverage proving renderer-first behavior and no duplicate OpenCode spawn; model-context coverage across failover. Run `node scripts/assert-work-session.mjs`, `node scripts/assert-model-failover.mjs`, `npx vitest run tests/work-session.test.ts tests/bot-host.test.ts`, `npx tsc --noEmit`, then the live BOT-19/lane/memory/soul gates from `docs/agent/DOMAIN_REGRESSION_MAP.md`.

**Live acceptance.** On VM, send one ordinary prompt after `/tx on`; `tmux capture-pane` on the exact target must visibly progress through thinking/tool status/usage/completion while Telegram receives the same run. Verify one OpenCode child per attempt, no prompt/tool payload in the observer log, `/abort` kills the agent but not the tail, `/tx off` does not kill the agent or unrelated `session 1`, repeated `/tx on` creates no duplicate observer, and bot restart preserves honest status. Only then mark BOT-19 done.
...
### Track V Phase 10 — Bug ticket pipeline for TG agentic development (V-30.1 DONE · V-30.2 DONE 2026-09-24 · V-30.3 DONE 2026-09-24 · V-30.4 DONE 2026-09-24 — V-30.5 NOT STARTED)
...
**Do not start** V-30.4…V-30.5 before an explicit human go. V-30.2 closed 2026-09-24 with
...
V-30.5's global strict capability gate is also currently red: five unrelated rows are
