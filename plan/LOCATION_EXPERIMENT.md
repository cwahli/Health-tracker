...
| `scripts/assert-swap-drill.mjs` (new, end-to-end) | 19 pass / 0 fail — real relay + real `runOnWorker`, 10 swaps, 1 injected wrong-ledger rollback, row never forked, plus the view repointed on a real isolated tmux server |
