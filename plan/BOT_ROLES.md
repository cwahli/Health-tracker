...
- **Binding (corrected 2026-09-22):** this bot is for the **Android device opencode** (Termux/proot, workspace `/root/Health-tracker`), not the VPS. Device token first lived in proot meal_audit `.env` (test: `opencode online`). A VPS `opencode-bot@android` was started mid-course and is the **wrong host** — disable it before polling from the phone. `@Opencode_135_bot` remains the VPS interactive door.
