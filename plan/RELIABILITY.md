...
Locked human decisions: one agent, called **`diet`** everywhere — canonical `diet` / `diet_answer` (locked 2026-09-13, supersedes `meal_agent`) · history stays, all future references (golden, tests, plan) update via **re-run, not hand-edit** · frozen `specs/checkpoints/*` stay immutable, new checkpoints cut afterward · `dietitian*` i18n keys renamed to `diet*` (en+id, parity test guards). `archive/` is out of scope for both purges. Scout-leg names (`scout`, `scout_answer`, `t1/scout`) are untouched.
...
Live emission points (re-stamp to `diet` / `diet_answer`):
...
Do **not**: delete the curator to “finish the purge”; put it back on Analyze; keep two names; reopen FDC. Wire id for the meal agent is **`diet`** (`diet_answer`, `t{n}/diet`, stage `diet`, i18n `diet*`); readers dual-accept `dietitian*` history.
