...
> **Status:** promoted to a locked packet — see [`specs/active/Q-11.md`](../specs/active/Q-11.md).
> This document is the design rationale; the packet is the enforced contract. Where the budget
> table below disagrees with the packet, the packet wins: the table here summed to 2,550 lines
> against a 9,101-line `src/App.tsx` and omitted auth/session, i18n and Header entirely, which is
> how the first attempt (commit `a14abea`) ended up stubbing `src/types.ts` and deleting
> `AuthScreen.tsx` to fit.
