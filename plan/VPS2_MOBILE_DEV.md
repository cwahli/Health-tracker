...
| **B. Termius → Mosh → tmux `health`** | **Watch and steer.** Attach to the live TUI (`grok`, Agy, OpenCode) in tmux session `health`. The session lives on the VM, so a phone drop does not kill the turn. Mosh talks to the public hostname and reconnects after Wi-Fi ↔ cellular. **V-27 server side built 2026-09-22** (mosh + UDP 60000:61000, tmux `health` + `tmux-health.service` user unit, key-only public :22, fail2ban sshd jail). Phone: switch Termius to the Mosh host and run the Wi-Fi ↔ cellular check; leave Tailscale SSH on until that passes. | When you need the real TUI, a stuck job, or to type into the CLI |
